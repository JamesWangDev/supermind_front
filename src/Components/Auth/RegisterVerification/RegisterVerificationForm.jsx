...
import useRegisterVerification from '@/Utils/Hooks/Auth/useRegisterVerification';
...
const RegisterVerificationForm = () => {
  ...
  const { mutate: registerVerification } = useRegisterVerification();
  ...
  useEffect(() => {
    otp && otp.length === 5 && registerVerification({ email: cookies, token: otp });
  }, [otp]);
  return (
    <>
      <Formik
